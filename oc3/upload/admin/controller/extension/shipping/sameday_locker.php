...
class ControllerExtensionShippingSamedayLocker extends Controller {
	private $error = [];
	...
	public function index() {
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/shipping/sameday_locker', 'user_token=' . $this->session->data['user_token'], true)
		];
		$data['action'] = $this->url->link('extension/shipping/sameday_locker', 'user_token=' . $this->session->data['user_token'], true);
		...
		$this->response->setOutput($this->load->view('extension/shipping/sameday_locker', $data));
	}

	public function install() {
		$this->load->model('extension/module/bookurier_setup');
		$this->model_extension_module_bookurier_setup->installShipping(Settings::SHIPPING_SAMEDAY_LOCKER_CODE);
	}
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/shipping/sameday_locker')) {
